...
function create_tunnel(index, ecbs) {
    let client = connect({
        ...
        //发送通道注册包
        let register_packet = gen_packet(0, 10, Buffer.allocUnsafe(32).toString(), Buffer.alloc(0));
        ...
        client._recv_handler = recv_handle(data => {
            ...
            if(pkt_type == 11) {
				...
				//结束连接回应
                let finish_ok_packet = gen_packet(0, 9, Buffer.allocUnsafe(32).toString(), Buffer.alloc(0));
        ...
        client.on("data", (data) => {
            ...
            if(client._authed == true) {
                ...
                let finish_ok_packet = gen_packet(0, 11, Buffer.allocUnsafe(32).toString(), Buffer.alloc(0));
                client.write(finish_ok_packet);
                client._authed = false;
                client.end();
                client.destroy();
