...
function init_input_tunnels(ecbs) {
    createServer({
        ...
        let recv_handler = recv_handle(data => {
            ...
            if(pkt_type == 10) {
                //发送通道注册成功包
                let register_packet = gen_packet(0, 11, Buffer.allocUnsafe(32).toString(), Buffer.alloc(0));
				...
				//断开连接
                setTimeout(() => {
                    let finish_packet = gen_packet(0, 8, Buffer.allocUnsafe(32).toString(), Buffer.alloc(0));
